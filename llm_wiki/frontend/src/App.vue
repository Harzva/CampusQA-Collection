<template>
  ...
  <header class="app-header">
    ...
    <!-- Mode selector: choose between RAG, Agent and LLM‑Wiki modes -->
...
async function uploadFile() {
  ...
  try {
    ...
    // LLM‑Wiki mode we upload to the wiki ingestion endpoint, otherwise we
    // upload to the standard RAG endpoint.
    const uploadEndpoint = selectedMode.value === 'llmwiki' ? '/api/wiki/upload' : '/api/documents';
...
async function sendQuestion() {
  ...
  try {
    ...
    // Agent uses /api/agent/chat, and LLM‑Wiki uses /api/wiki/chat.
    let endpoint;
